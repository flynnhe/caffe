...
void IdctEuclideanLossLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  // compute the idct2
  caffe_gpu_idct2(num_examples, npix, num_blocks, block_size, c_coeffs0, c_coeffs1, all_pixels0, all_pixels1);
    ...
    if (propagate_down[i]) {
      ...
      for (int j = 0; j < N; ++j) {
        ...
        for (int k = 0; k < num_blocks; ++k) {
          ...
          caffe_gpu_gemm<Dtype>(CblasTrans, CblasNoTrans, 1, block_size, block_size,
